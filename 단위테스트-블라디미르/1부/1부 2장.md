...
- 단위의 크기: 단일 클래스
- 테스트 대역(Test double) 사용 대상: 불변 의존성 외 모든 의존성
...
- 테스트 대역(Test double) 사용 대상: 공유 의존성
...
- TDD를 통한 시스템 설계 방식 차이
  - 런던파: 하향식(탑다운) 설계. 상위 레벨 테스트부터 시작하고, 하위는 대역으로 대체하여 나중에 구현할 수 있다.
  - 고전파: 테스트에서 실제 객체를 사용해야 하기 때문에 일반적으로 상향식(바텀업)으로 구현한다. 도메인 모델을 시작으로 구현하는 편이다.
- 테스트 코드와 SUT 결합도 차이
  - 런던파: 클래스 단위로 테스트하다 보니, 고전파보다 테스트코드가 SUT와 더 강하게 결합된다. 또한 모든 의존성에 테스트 대역을 사용하여, 테스트 코드에 구현 세부 사항이 결합된다.
- 오버 엔지니어링
  - 고전파: 상향식(바텀업)으로 개발하다 보니 나중에 필요하지 않은 코드가 많이 생기는, 오버 엔지니어링이 될 수 있다.
